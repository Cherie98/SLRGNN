...
class SE_GNN(nn.Module):
    def __init__(self, h_dim):
        ...
        entity_embedding = np.load("../outputs/FB15k237/entity_embedding.npy")
        tensor_ent_emb = torch.from_numpy(entity_embedding)
        covariance_matrix = torch.matmul(tensor_ent_emb.T, tensor_ent_emb)
        eigenvalues, eigenvectors = torch.symeig(covariance_matrix, eigenvectors=True, upper=True)
        eigenvalues, indices = eigenvalues.sort(descending=True)
        eigenvectors = eigenvectors[:, indices]
        k = 1000
        principal_components = eigenvectors[:, :k]
        self.ent_emb = torch.matmul(tensor_ent_emb, principal_components)
